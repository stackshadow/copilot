...
// debug
    ...
    if( this->onJsonMessage != NULL ){
        ...
        // call callback
            ...
            // debug
                snprintf( etDebugTempMessage, etDebugTempMessageLen, "[%p]: [%s] -> [%s]/[%s] -> [%s] call [%p]%s",
                objectSource, msgNodeSource, msgGroup, msgCommand, nodeTarget, this->onMessage, "(onMessage)" );
                etDebugMessage( etID_LEVEL_DETAIL_APP, etDebugTempMessage );
        ...
        //  call json-callback
            ...
            // debug
                snprintf( etDebugTempMessage, etDebugTempMessageLen, "[%p]: [%s] -> [%s]/[%s] -> [%s] call [%p]%s",
                objectSource, msgNodeSource, msgGroup, msgCommand, nodeTarget, this->onJsonMessage, "(onJsonMessage)" );
                etDebugMessage( etID_LEVEL_DETAIL_APP, etDebugTempMessage );
...
/**
 * @brief This function create a NEW json_t object. You MUST free this object with json_decref()
 * @param jsonObject
 * @param id
 * @param nodeSource
 * @param nodeTarget
...
void psBus::                _subscribe( void* objectSource, const char* nodeTarget, const char* group, void* userdata, psSubscriberMessage callback, psSubscriberJsonMessage jsonCallback ){
...
void psBus::                _publish( void* objectSource, const char* id, const char* nodeSource, const char* nodeTarget, const char* group, const char* command, const char* payload ){
	lockPthread(this->psSubscriberArrayLock);
...
void psBus::                _publish( void* objectSource, json_t* jsonObject ){
    lockPthread(this->psSubscriberArrayLock);
...
void psBus::                _publishOrSubscribe( void* objectSource, json_t* jsonObject, void* userdata, psSubscriberMessage callback, psSubscriberJsonMessage jsonCallback ){
...
// check if we get an subscribe-message
    if( strncmp(msgGroup,"bus",3) == 0 && strncmp(msgCommand,"subscribe",9) == 0 ){
...
// publish it
    this->_publish( objectSource, jsonObject );
}
